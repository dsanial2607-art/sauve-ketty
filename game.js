...
const music=$('#music'); music.volume=.45;
...
function render(){renderWord();renderKeyboard();$('#batteryImg').src=`assets/images/battery/${String(lives*10).padStart(2,'0')}.png`;$('#hintBtn').classList.toggle('used',hintUsed);$('#hintBtn img').src=hintUsed?'assets/images/buttons/hint_done.png':'assets/images/buttons/hint.png'}
function renderWord(){const box=$('#word');box.innerHTML='';[...word].forEach(ch=>{const slot=document.createElement('div');slot.className='letter-slot';if(guessed.has(ch)){slot.innerHTML=`<div class="letter">${ch.toUpperCase()}</div><img class="underline" src="assets/images/underline_yes.png">`}else{slot.innerHTML=`<img class="blur" src="assets/images/letter_blur.png"><img class="underline" src="assets/images/underline_no.png">`}box.appendChild(slot)})}
function renderKeyboard(){const kb=$('#keyboard');kb.innerHTML='';const rows=['ABCDEFGHIJ','KLMNOPQRS','TUVWXYZ'];rows.forEach(r=>{const row=document.createElement('div');row.className='key-row';[...r].forEach(L=>{const l=L.toLowerCase(),b=document.createElement('button');b.className='key';let path=`assets/images/keyboard/normal/${L}.png`;if(guessed.has(l))path=`assets/images/keyboard/right/${L}-right.png`;if(wrong.has(l))path=`assets/images/keyboard/false/${L}-false.png`;if(eliminated.has(l)){b.classList.add('disabled');path=`assets/images/keyboard/false/${L}-false.png`}b.innerHTML=`<img src="${path}" alt="${L}">`;b.disabled=guessed.has(l)||wrong.has(l)||eliminated.has(l);b.addEventListener('click',()=>pick(l));row.appendChild(b)});kb.appendChild(row)})}
...
function applyCustomImages(){
  ...
  document.querySelector('.intro-screen').style.backgroundImage=`url("${intro||'assets/images/sauve_ketty_intro.png'}")`;
}
function getWinImage(){return localStorage.getItem('sauveKettyWin')||'assets/images/sauve_ketty_win.png'}
...
finish=function(win){idleTimers.forEach(clearTimeout);music.pause();show('end');$('#endImage').src=win?getWinImage():'assets/images/sauve_ketty_lose.png';$('#endText').textContent=win?'Bravo ! Tu as trouvé le bon mot ! Ketty est réparée !':`Le mot à trouver était « ${word.toUpperCase()} ». Heureusement, Ketty est incassable !`;if(win){play('#sfxWin');say('Bravo ! Tu as trouvé le bon mot ! Je suis complètement réparée ! Merci.')}else{play('#sfxLose');say(`Ouf ! Heureusement que je suis incassable ! Le mot à trouver était ${word}.`)}}
